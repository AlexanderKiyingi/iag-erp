...
func (s *Store) ClaimBatch(ctx context.Context, limit int, backoff time.Duration) ([]Row, error) {
	...
	rows, err := s.pool.Query(ctx, `
		WITH due AS (
			SELECT id FROM erp_event_outbox
			WHERE dispatched_at IS NULL AND available_at <= NOW()
			ORDER BY id
			FOR UPDATE SKIP LOCKED
			LIMIT $1
		)
		UPDATE erp_event_outbox o
...
func (p *Publisher) Run(ctx context.Context) {
	...
	for {
		...
		case <-ticker.C:
			...
			if err != nil {
				slog.Warn("erp outbox drain", "err", err)
				continue
			}
			if n >= p.batch {
				_, _ = p.drainOnce(ctx)
...
func (p *Publisher) drainOnce(ctx context.Context) (int, error) {
	...
	for _, r := range rows {
		if err := p.dispatcher.DispatchOutbox(ctx, r); err != nil {
			...
			slog.Warn("erp outbox dispatch failed", "id", r.ID, "type", r.EventType, "err", err)
			continue
		}
