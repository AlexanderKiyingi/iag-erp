...
import (
	...
	"iag-erp/backend/internal/models"
)
...
func registerPermissionsLoop(ctx context.Context, cfg *config.Config) {
	...
	for {
		...
		err := platformserviceauth.RegisterPermissions(regCtx, saClient, cfg.JWTIssuer, "erp", perms)
		c()
		if err == nil {
			log.Printf("erp: registered %d permissions", len(perms))
			return
		}
		log.Printf("erp: permissions register failed: %v (retry in %s)", err, backoff)
		select {
		case <-ctx.Done():
			return
